...
const RoiDetailPanel = ({
  ...
  const roleLabel = isBackground ? '背景' : (isReference ? '基准' : `样品 ${index + 1}`);

  return (
    <div className="flex h-full flex-col rounded-[18px] border border-slate-200 bg-slate-50/70 p-2.5">
      <div>
        <div className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">ROI 详情</div>
        ...
        <div className="mt-1.5 flex flex-wrap gap-1.5">
          ...
          <span className={`rounded-full border px-2 py-0.5 text-[10px] font-black ${toneClasses[saturationRisk.tone]}`}>{saturationRisk.level}</span>
        </div>
      </div>

      <div className="mt-2.5 space-y-1.5">
        <div className={itemClass}>
          <div className="text-[9px] font-black uppercase tracking-widest text-slate-400">强度</div>
        ...
        <div className={itemClass}>
          <div className="text-[9px] font-black uppercase tracking-widest text-slate-400">饱和比例</div>
          <div className="mt-0.5 text-[13px] font-mono font-black text-slate-900">{saturationMetrics.percentage.toFixed(2)}%</div>
        </div>
        <div className={itemClass}>
          <div className="text-[9px] font-black uppercase tracking-widest text-slate-400">饱和像素</div>
          <div className="mt-0.5 text-[13px] font-mono font-black text-slate-900">{saturationMetrics.saturatedPixels}</div>
        </div>
        <div className={itemClass}>
          <div className="text-[9px] font-black uppercase tracking-widest text-slate-400">状态</div>
          <div className="mt-0.5 text-[12px] font-black text-slate-900 break-words">{saturationRisk.level}</div>
        </div>
      </div>
      ...
      <div className="mt-2 space-y-1.5">
        ...
        {saturationMetrics.percentage > 0 && (
          <div className="flex items-start gap-2 rounded-xl border border-slate-200 bg-white px-2.5 py-2 text-[10px] leading-4 text-slate-500">
            ...
            <span>饱和比例升高会压缩真实强度差异，当前结果更适合参考性比较。</span>
          </div>
        )}
      </div>
